...
from social.base import INDICATOR_CODES
...
def parse_hcp_indicator(code: str) -> pl.DataFrame | None:
    ...
    # Parse data keys: format is "{dim0_id}.{dim1_id}_{period}" or "{dim0_id}_{period}"
    rows = []
    for key, val_obj in raw_data.items():
        ...
        # Split key - handle both 1-dim and 2-dim cases
        try:
            if "." in key.split("_")[0]:
...
def parse_all_indicators() -> pl.DataFrame:
    """Parse all downloaded HCP BDS social indicators."""
    frames = []
...
if __name__ == "__main__":
    print("=== Parsing HCP BDS social indicators ===")
