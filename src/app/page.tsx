...
function KPICard({
  ...
  return (
    ...
    >
      <Card className="border-l-4 p-5" style={{ borderLeftColor: color }}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            >
              <ModuleIcon className="h-4.5 w-4.5" style={{ color }} />
            </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <span className={`text-xs font-medium ${trendColor(kpi.trend as 'up' | 'down' | 'stable')}`}>
              {kpi.trendLabel}
            </span>
          </div>
        </div>
        <div className="mt-3 flex items-baseline gap-1.5">
          <span className="text-2xl font-bold tracking-tight">{typeof kpi.value === 'string' ? kpi.value : formatKPIValue(kpi.value, kpi.indicator?.code || '')}</span>
          {kpi.unit && (
            <span className="text-sm font-medium text-muted-foreground">{kpi.unit}</span>
          )}
        </div>
        <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{t(kpi.description, lang)}</p>
      </Card>
    </motion.div>
  )
}
...
function ModuleContent({
  ...
  const periodKPIs = useMemo(() => {
    ...
    return module.kpis.map((kpi) => {
      const matchingIndicator = module.indicators.find((ind) => {
        if ((kpi as any).indicatorCode && ind.code === (kpi as any).indicatorCode) return true
          ...
          {activeChart === 'line' && (
            <ResponsiveContainer width="100%" height={350}>
              <LineChart data={allPts}>
                ...
                <ReferenceLine y={avg} stroke="#a855f7" strokeDasharray="5 5" label={{ value: `Moy: ${formatKPIValue(avg, ind.code)}`, position: 'right', fill: '#a855f7', fontSize: 11 }} />
